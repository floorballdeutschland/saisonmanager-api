...
- **Fehlende Ressourcen-Scope-Prüfungen in Admin-Endpunkten (IDOR)**: Mehrere Admin-Endpunkte prüften zwar die Rolle des Nutzers, aber nicht, ob die konkret angefragte Ressource in seinem Zuständigkeitsbereich liegt. Betroffen und behoben:
  ...
  - **Schiri-Bericht (`GET games/:game_id/referee_report`)**: Der Upload war auf die angesetzten Schiris beschränkt, der Abruf der Bericht-Blob-URL jedoch für jeden Login möglich. Der Abruf ist jetzt auf Admin, SBK im Spielbetrieb des Spiels sowie die angesetzten Schiris beschränkt.
- **Vereinsmanager konnte verbandsübergreifend jedes Spiel bearbeiten**: In `GamesController#set_string` (Live-Erfassung) lieferte der erste `intersection(...)`-Ausdruck der VM-Prüfung ein Array zurück; ein leeres Array ist in Ruby truthy, sodass die `||`-Kette kurzschloss und **jeder** VM Ereignisse **jedes** Spiels (auch anderer Landesverbände) setzen konnte. Der Treffer wird jetzt mit `.present?` ausgewertet; Platzhalter-Spiele ohne Heim-/Gastmannschaft werden nil-sicher behandelt.
