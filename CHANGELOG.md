...
- **Altdaten-Import – Lizenzen (PoC, intern)**: `*_lizenz` + `*_lizenzverlauf` werden zu `players.licenses`-Einträgen (`team_id`, `league_class_id`, `league_category_id`, chronologische `history` mit `license_status_id`/`created_at`) und idempotent (Schlüssel `LIC:<verband>:<saison>:<id_lizenz>`) in die jeweiligen Spieler gemergt (neue Phase 3 der Saison-Transaktion). Lizenzstatus mappt 1:1 (alt 1–6 == `License::APPROVED..TRANSFER`). Export-SQL/`legacy:league` liefern beide Tabellen mit; `Transformer.license_attrs` ist unit-getestet.
