...
### Verbessert
- **Ladezeit Tabelle/Scorer/Gruppen-Tabelle**: `GET /leagues/:id/table`, `/grouped_table` und `/scorer` werden jetzt 5 min gecacht (`Rails.cache`, analog zum bereits gecachten `schedule`). Diese Endpunkte parsten bisher bei **jedem** Aufruf die JSONB-`events`-Spalte sämtlicher Spiele neu (Ergebnis-/Scorer-Berechnung in Ruby) – bei großen Ligen der teuerste Read. Die Caches werden bei Ergebnis-/Aufstellungsänderungen über denselben Mechanismus invalidiert wie der Spielplan (`Game.autofill_teams!`); das Caching liegt im Controller, der modellinterne `League#grouped_table`-Aufruf der Cup-Befüllung bleibt uncached/frisch.
