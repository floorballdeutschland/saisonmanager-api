...
- **Schiri-Lizenzliste – Saisonspiele**: Die Übersicht `GET admin/referees` liefert je Schiedsrichter jetzt `season_game_count` (Anzahl der Spiele in der aktuellen Saison, gezählt analog zu `Referee#games` über `referee_ids` bzw. Lizenznummer-Präfix in `referee1/2_string`). Die Zahl wird in **einer** Aggregations-Query über die Saisonspiele ermittelt (keine N+1-Counts über die Liste). Grundlage für die neue Spalte „Saisonspiele" im Frontend.
