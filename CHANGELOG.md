...
### Behoben
- Ligaverwaltung: Die Verbands-Logos fehlten, seit der Fallback auf die alte `logo_url`-Textspalte entfernt wurde (`GameOperation#meta_hash`). Da bislang nur wenige Landesverbände ein Logo als Anhang hinterlegt haben, wurde `logo_url` für die übrigen Verbände auf `nil` gesetzt. Das attached LV-Logo hat weiterhin Vorrang, fällt aber wieder auf das vorhandene Textspalten-Logo zurück
