...
- Schiedsrichteransetzungen: Optionale Ansetzung eines **Schiedsrichtercoachs** je Spiel (`referee_assignments.coach_id`). Als Coach ansetzbar sind Personen mit gültiger Beobachtungs-Zusatzlizenz (Qualifikationstyp `B…`, `valid_until` leer oder in der Zukunft) – diese sind immer auch Schiedsrichter. Neuer Endpoint `GET /api/v2/admin/referee_assignments/available_coaches?date=…` liefert die Coach-Kandidaten ohne Sperrtermin am Spieltag; `coach_id` ist über `POST/PUT …/referee_assignments` pflegbar. Ein angesetzter Coach wird in der Veröffentlichungs-E-Mail an die Schiedsrichter mit aufgeführt (neuer Platzhalter `{{coach_name}}`).
