...
class PlayersControllerTest < ActionDispatch::IntegrationTest
  ...
  # ---------------------------------------------------------------------------
  # transfer / add_additional_club: Spielbetriebs-Scope (#398)
  #
  # Beide Aktionen prüften nur, OB jemand eine Spielbetriebsrolle hat, nicht
  # WELCHEN Spielbetrieb. Der Transfer schreibt dabei einen Eintrag mit
  # `home_club: true` – wer ihn ausführt, ist danach regulär zuständig und
  # passiert jede weitere Prüfung. Die Regeln aus #391 und #394 begrenzten damit
  # nur den bequemen Weg, nicht den Zugriff.
  # ---------------------------------------------------------------------------

  test 'transfer: fremde SBK kann kein fremdes Profil in den eigenen Verband ziehen' do
    player = player_homed_in(@game_operation)
    ...
    assert_response :forbidden
    assert_equal 1, home_club_ids(player).size,
  ...
  # Eigener Spieler statt @player aus dem Setup: Die Fabrik :club setzt KEINEN
  # Heimat-Spielbetrieb, main_game_operation_id bliebe nil und damit ausserhalb
  # jedes SBK-Scopes. club_in/fremder_spielbetrieb gibt es oben schon.
