...
# Der Schalter selbst: Er laesst den Nachnamen aus der oeffentlichen Ausgabe
# weg und ruehrt den Datensatz nicht an. Genau das ist der Punkt der Loesung --
# ohne Name und Geburtsdatum im Bestand waere dieselbe Person jederzeit ein
# zweites Mal anlegbar.
class PlayerPublicLastNameHiddenTest < ActiveSupport::TestCase
  setup do
    @user = create(:user, :admin)
    @player = create(:player, first_name: 'Pierre', last_name: 'Beispiel')
  end

  test 'der Schalter laesst Name und Geburtsdatum im Datensatz stehen' do
    @player.hide_public_last_name!(@user.id, reason: 'DSGVO 2026-09-22')

    @player.reload
    assert @player.public_last_name_hidden?
    ...
    assert_equal @user.id, @player.public_last_name_hidden_by
    assert_equal 'DSGVO 2026-09-22', @player.public_last_name_hidden_reason
  end

  test 'Zuruecknehmen raeumt die Kennzeichnung ab' do
    @player.hide_public_last_name!(@user.id, reason: 'DSGVO 2026-09-22')
    @player.show_public_last_name!(@user.id)

    @player.reload
    assert_not @player.public_last_name_hidden?
    assert_nil @player.public_last_name_hidden_at
  end

  test 'ein Profil, das die heutigen Validierungen nicht erfuellt, laesst sich schalten' do
    # Altbestand ohne nation_id: Ein Betroffenenantrag darf daran nicht
    # scheitern, deshalb speichert #hide_public_last_name! ohne Validierung.
    @player.update_column(:nation_id, nil)

    assert_nothing_raised { @player.reload.hide_public_last_name!(@user.id) }
    assert Player.public_last_name_hidden.exists?(@player.id)
  end

  test 'leerer Vermerk wird nicht als Leerzeichenkette gespeichert' do
    @player.hide_public_last_name!(@user.id, reason: '   ')

    assert_nil @player.reload.public_last_name_hidden_reason
  end

  test 'PublicPlayerNames trifft nur die betroffene Spieler-ID' do
    other = create(:player)
  ...
  test 'ein Aufstellungseintrag wird kopiert, nicht im Bestand ueberschrieben' do
    ...
    # Vorname und Trikotnummer bleiben: Weggelassen wird allein der Nachname.
    assert_equal 'Pierre', masked['player_firstname']
  ...
  test 'ohne geschaltetes Profil kommt die Aufstellung unveraendert zurueck' do
    entries = [{ 'player_id' => @player.id, 'player_name' => 'Beispiel' }]

    assert_equal entries, PublicPlayerNames.mask_lineup(entries)
  end

  test 'die ganze Aufstellungsseite verliert die betroffenen Nachnamen' do
    ...
    assert_equal %w[Pierre Anna], masked.pluck('player_firstname')
  end
  ...
  # Die Stufe ist ausdruecklich "Nachname weg", nicht "Initiale". Ein Rest wie
  # "Pierre L." waere in derselben Liga trivial zurueckzufuehren.
  test 'es bleibt kein Rest des Nachnamens stehen' do
    @player.hide_public_last_name!(@user.id)

    _first, last = PublicPlayerNames.mask_names(@player.id, 'Pierre', 'Beispiel')
  ...
  test 'das Umschalten leert den Satz der betroffenen IDs' do
    with_real_cache do
