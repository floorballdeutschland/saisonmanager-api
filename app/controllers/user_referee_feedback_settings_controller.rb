...
class UserRefereeFeedbackSettingsController < ApplicationController
  ...
  def update
    ...
    team.update!(
      feedback_contact_email: email.presence,
      feedback_contact_prefer_captain: prefer_captain_param,
  ...
  def prefer_captain_param
    ActiveModel::Type::Boolean.new.cast(params[:feedback_contact_prefer_captain]).present?
  end

  # Nur Mannschaften, die überhaupt in einer feedback-pflichtigen Liga spielen
  # (inkl. Pokal-/Zusatzligen über Team#all_league_ids) – analog zum Menü-Gate
  # User#manages_referee_feedback_team?.
  def settings_teams
    enabled_league_ids = League.where(referee_feedback_enabled: true).pluck(:id)
